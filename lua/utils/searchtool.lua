...
local current_search_engine = "duckduckgo"
...
M.web_search = function(query, engine)
...
vim.api.nvim_create_user_command("SearchWith", function(opts)
  ...
  if not search_engines[engine] then
    vim.notify("Unknown search engine: " .. engine .. ", using default", vim.log.levels.ERROR, { title = "Web Search" })
    vim.notify(
      "Searching on web using " .. current_search_engine .. ": " .. query,
      vim.log.levels.INFO,
      { title = "Web Search" }
...
-- TODO: how to load below three functions without having error at startup.
-- =========================================
--  Diagnostic and Selection Based Search
-- =========================================

-- Grabs the diagnostic under cursor and searches it with language context
-- M.search_diagnostic_under_cursor = function()
...
-- M.search_selected_text = function()
